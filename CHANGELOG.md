...
## 2026-05-17 — v2.0: Categorized library + 20 new skills

### Restructure
- Reorganized the flat `skills/` directory into 7 category subfolders for easier discovery and navigation:
  `financial-analysis/`, `product-management/`, `strategic-management/`, `game-theory/`, `engineering/`, `data-analysis/`, `document-processing/`
...
- `capital-allocation-framework` — NPV / IRR / PI / EAA + strategic option value across a portfolio frontier
- `burn-rate-runway-planner` — burn decomposition, scenario plans, 13-week cash forecast, fundraise triggers
- `financial-statement-analyzer` — 10-K / 10-Q teardown with margin waterfall and accounting red-flag log

### New skills — Product Management (5)
- `jobs-to-be-done-extractor` — JTBD job statements, job map, outcome scoring, hire / fire analysis
- `opportunity-solution-tree` — Teresa Torres OST connecting outcomes to opportunities and assumption tests
- `product-roadmap-prioritizer` — RICE + Kano + MoSCoW + Cost of Delay convergence analysis
...
- `wardley-mapping` — value chain on the evolution axis with climatic patterns, doctrine, gameplay
- `swot-tows-analyzer` — evidence-anchored SWOT + TOWS producing prioritized options
- `okr-cascade-planner` — company → function → team OKR cascade with quarterly rituals

### New skills — Game Theory (5)
- `nash-equilibrium-solver` — pure / mixed equilibria, dominance, subgame perfection, selection
