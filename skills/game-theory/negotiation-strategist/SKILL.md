...
> Preparation collapses the role of luck. This skill is the preparation.
...
- Walking into negotiations with a target price but no BATNA
...
Single-issue negotiation is always distributive (zero-sum). Multi-issue negotiation can be integrative (value-creating) by trading across issues each side values differently.

For each side, list all issues and rank by importance (1 = most, n = least):
...
Pre-game with a colleague playing the counterparty. Specifically:
- Run the opening move and three rounds
- Pressure-test your concession plan
- Identify the one question that would derail you and prepare an answer
- Identify your tells (impatience, over-justification) and plan around them
...
**Always**
- Define BATNA and reservation price before the meeting
- Convert positions into interests
- Expand to multi-issue when possible
- Make conditional concessions (never unilateral)
- Simulate the counterparty before the real conversation

**Never**
- Open with a midpoint number
- Negotiate against yourself with successive concessions
- Reveal your reservation price
- Treat their first offer as their reservation price
- Confuse a tactic ("higher authority") with a real constraint
